...
public:
  Option()
      : _action("store"), _type("string"), _nargs(1), _suppress_help(false),
        _callback(0) {}
  ...
  template <typename T> Option &set_default(T t) {
...
private:
  ...
  friend OptionParser &
  detail::add_option_group_helper(OptionParser &parser,
                                  const OptionGroup &group);
};
...
public:
  OptionParser()
      : _usage("%prog [options]"), _add_help_option(true),
        _add_version_option(true), _interspersed_args(true) {}
